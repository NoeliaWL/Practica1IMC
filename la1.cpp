...
int main(int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, "t:T:i:l:h:e:m:s::w:p")) != -1)
    {
        ...
        switch(c){
            ...
            case 'p':
                ...
                break;
            case '?':
                if (optopt == 't' || optopt == 'T' || optopt == 'i' || optopt == 'l' || optopt == 'h' || optopt == 'e' || optopt == 'm' || optopt == 'w' || optopt == 'p')
    ...
    if (!pflag) {
        ...
        int neuronsCapaOculta = hflag ? atoi(hvalue) : 5;
        
        topology[0] = trainDataset->nOfInputs;
        topology[layers+1] = testDataset->nOfOutputs;
        for(int i=1; i<layers+1; i++){
            topology[i] = neuronsCapaOculta;
        }
        ...
        for(int i=0; i<5; i++){
            ...
            varianzaTest = varianzaTest + rangoTest;
        }
        varianzaTrain = varianzaTrain / 5;
        varianzaTest = varianzaTest / 5;
